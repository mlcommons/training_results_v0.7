...
using FChangeLayout = std::function<mshadow::LayoutFlag(nnvm::NodeAttrs*,
                                                        mshadow::LayoutFlag,
                                                        std::vector<nnvm::TShape>*,
                                                        std::vector<nnvm::TShape>*)>;

static int fillAllAxes(std::vector<nnvm::TShape>* inpTransposes,
                       std::vector<nnvm::TShape>* outTransposes) {
  nnvm::TShape inpAxes;
  nnvm::TShape outAxes;
  if (!inpTransposes->empty()) {
    for (auto& ax : *inpTransposes) {
      if (ax.ndim() > 0) {
        inpAxes = ax;
      }
    }
  ...
  if (!outTransposes->empty()) {
    for (auto& ax : *outTransposes) {
      if (ax.ndim() > 0) {
        outAxes = ax;
      }
    }
  }
  if (inpAxes.ndim() > 0 && outAxes.ndim() > 0) {
    bool accept = inpAxes.ndim() == outAxes.ndim();
    for (size_t i = 0; i < std::min(inpAxes.ndim(), outAxes.ndim()); i++)
      accept *= inpAxes[i] == common::ReverseTransposeAxes(outAxes)[i];
    if (!accept) {
      inpTransposes->clear();
      outTransposes->clear();
      return -2;
    }
  } else if (inpAxes.ndim() > 0) {
    outAxes = common::ReverseTransposeAxes(inpAxes);
  } else if (outAxes.ndim() > 0) {
    inpAxes = common::ReverseTransposeAxes(outAxes);
  } else {
    inpTransposes->clear();
    outTransposes->clear();
    return -1;
  }
  inpTransposes->assign(inpTransposes->size(), inpAxes);
  outTransposes->assign(outTransposes->size(), outAxes);
  return 0;
}
