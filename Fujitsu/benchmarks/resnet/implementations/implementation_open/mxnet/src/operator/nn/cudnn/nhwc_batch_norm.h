...
class NhwcBatchNorm {
 public:
  ...
  NhwcBatchNorm() {
    ...
    init_params(MAX_PIXELS_PER_THREAD_IN_SMEM_FWD);
  }
  ...
  inline void fwdInference(cudaStream_t stream, bool use_relu);
  inline void init_params(int new_pixels_per_thread_in_smem_fwd);
  ...
  inline const std::vector<size_t> numWorkspaceBytes() const;
 ...
 protected:
  ...
  static const int MAX_PIXELS_PER_THREAD_IN_SMEM_FWD = 10;
...
#define LAUNCH_BWD_RELU_KERNEL(OUTER_LOOPS, COMPILED_FOR_OCCUPANCY) \
    do { \
        ...
        auto bwd_relu_func = nhwc_batch_norm_bwd_relu< \
                        ...
                        COMPILED_FOR_OCCUPANCY>; \
        if (COMPILED_FOR_OCCUPANCY > 1) { \
            cudaFuncSetAttribute(bwd_relu_func, \
...
#undef LAUNCH_BWD_KERNEL
 ...
 private:
  ...
  size_t launch_margin(int sm_arch, size_t full_gpu_grid_x) {
    static bool warning_logged = false;
    static int lm_env = dmlc::GetEnv("NHWC_BATCHNORM_LAUNCH_MARGIN", -1);
    int lm_default = (sm_arch < 70) ? 0 : NHWC_BATCHNORM_LAUNCH_MARGIN_DEFAULT;
    size_t lm_min = (sm_arch < 70) ? 0 : NHWC_BATCHNORM_LAUNCH_MARGIN_MIN;
  ...
  size_t max_fwd_grid_x(int device_id) {
    ...
    return full_gpu_grid_x - launch_margin(SMArch(device_id), full_gpu_grid_x);
  }
  ...
  int smem_driven_fwd_occupancy(int device_id) {
    ...
    return capped_occupancy(occupancy);
  }
...
dim3 NhwcBatchNorm::calc_fwd_grid(int device_id, int *loop, bool use_group_bn) {
  ...
  static bool reduce_quantization = dmlc::GetEnv("MXNET_CUDNN_NHWC_BN_QUANT_RED", false);
  if (grid_dim.x <= max_grid_x) {
    *loop = 1;
    if (max_grid_x / grid_dim.x >= 1) {
      grid_dim.y = std::min(c_blks, static_cast<int>(max_grid_x / grid_dim.x));
      if (reduce_quantization && grid_dim.y == 1) {
          int new_pixels_per_thread_in_smem_fwd = max((static_cast<int>(
                          (m_ * THREADS_PER_PIXEL) / THREADS_PER_CTA / max_grid_x)
                      - PIXELS_PER_THREAD_IN_REGISTERS_FWD), 0);
          if (new_pixels_per_thread_in_smem_fwd > 0) {
              init_params(new_pixels_per_thread_in_smem_fwd);
...
void NhwcBatchNorm::init_params(int new_pixels_per_thread_in_smem_fwd) {
  ...
  smem_size_fwd = pixels_per_thread_in_smem_fwd * THREADS_PER_CTA *
      ELEMENTS_PER_LDG*sizeof(uint16_t);
  pixels_per_cta_fwd = THREADS_PER_CTA / THREADS_PER_PIXEL *
...
void NhwcBatchNorm::fwd(cudaStream_t stream, bool use_relu, int device_id) {
  ...
  params.my_data = params.pair_data = params.pair_data2 = 0;
  ...
  // currently supporting only up to bn_group=4 (sync_iters=2)
  // to avoid any "grouping" sync_iters should be ==0
  params.sync_iters = 0;
  if (bn_group_ > 1) {
      params.my_data = xbuf_ptr_[device_id];
      params.pair_data = xbuf_ptr_[device_id^1];
      magic_ = (magic_+1)&0xff;   // magic number is used to indicate "fresh" data readiness
      params.magic = magic_;
      params.sync_iters = 1;
      if (bn_group_ > 2) {
          params.pair_data2 = xbuf_ptr_[device_id^2];
          params.sync_iters = 2;
      }
  }
...
void NhwcBatchNorm::dgrad(cudaStream_t stream, bool use_relu, int device_id) {
  ...
  if (bn_group_ > 1) {
      ...
      params.pair_data = xbuf_ptr_[device_id^1];
      params.magic = magic_;
      params.wgrad_coeff = 0.5;   // dscale and dbiad will be adjusted using this
