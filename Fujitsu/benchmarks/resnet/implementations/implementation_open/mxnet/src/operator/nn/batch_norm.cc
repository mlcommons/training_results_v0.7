...
static mshadow::LayoutFlag BNChangeLayout(nnvm::NodeAttrs* attrs,
                                          mshadow::LayoutFlag targetLayout,
                                          std::vector<nnvm::TShape>* inpTransposes,
                                          std::vector<nnvm::TShape>* outTransposes) {
  const BatchNormParam& param_ = nnvm::get<BatchNormParam>(attrs->parsed);
  int ax = param_.axis;
  // if there's more than 1 input delete it (it's invalid)
  inpTransposes->resize(1);
  // we need to have at leat 1 out transpose axes for `fillAllAxes` function
  if (outTransposes->empty())
    outTransposes->resize(1);
  if (alm::fillAllAxes(inpTransposes, outTransposes) != 0)
    return mshadow::kUNKNOWN;
  // downsize vector to delete all, now unnecessary, connections
  outTransposes->resize(1);
  // fill vectors with empty axes to satisfy connections that shouldn't be transposed
  auto n_inps = attrs->op->num_inputs;
  inpTransposes->resize(n_inps);
  auto n_outs = attrs->op->num_outputs;
  outTransposes->resize(n_outs);
  // set proper axis of work
  attrs->dict["axis"] = std::to_string(outTransposes->at(0)[ax]);
  return common::Transpose(targetLayout, outTransposes->at(0));
}
