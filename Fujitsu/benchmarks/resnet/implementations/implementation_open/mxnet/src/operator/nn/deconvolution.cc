...
static mshadow::LayoutFlag DeconvChangeLayout(nnvm::NodeAttrs* attrs,
                                            mshadow::LayoutFlag targetLayout,
                                            std::vector<nnvm::TShape>* inpTransposes,
                                            std::vector<nnvm::TShape>* outTransposes) {
  // get current layout
  const DeconvolutionParam& param_ = nnvm::get<DeconvolutionParam>(attrs->parsed);
  mshadow::LayoutFlag currentLayout = mshadow::kUNKNOWN;
  if (param_.layout)
    currentLayout = static_cast<LayoutFlag>(*param_.layout);
  else if (attrs->dict.find("layout") != attrs->dict.end())
    currentLayout = mshadow::layoutFlag(attrs->dict["layout"]);
  CHECK_NE(currentLayout, mshadow::kUNKNOWN) << "Current layout of deconvolution should be known";
  const size_t N_INPS_NO_BIAS = 2;
  auto n_outs = attrs->op->num_outputs;
  inpTransposes->resize(N_INPS_NO_BIAS);
  if (outTransposes->size() < n_outs)
    outTransposes->resize(n_outs);
  // if target layout known infer transposes based on it
  if (targetLayout != mshadow::kUNKNOWN) {
    auto v_axes = mshadow::getTranspAxes<nnvm::dim_t>(currentLayout, targetLayout);
    for (size_t i = 0; i < inpTransposes->size(); i++)
      inpTransposes->at(i) = nnvm::TShape(v_axes);
    v_axes = mshadow::getTranspAxes<nnvm::dim_t>(targetLayout, currentLayout);
    for (size_t i = 0; i < outTransposes->size(); i++)
      outTransposes->at(i) = nnvm::TShape(v_axes);
  // if not, maybe one of the axes have information about targets
  } else if (alm::fillAllAxes(inpTransposes, outTransposes) != 0) {
    targetLayout = common::Transpose(currentLayout, inpTransposes->at(0));
  } else {
    inpTransposes->clear();
    outTransposes->clear();
  ...
  outTransposes->resize(n_outs);
  if (!param_.no_bias) {
    inpTransposes->emplace_back();  // Bias does not need transposing
  }
  // change layout (still needs to be parsed to change `_param`)
  attrs->dict["layout"] = mshadow::toString(targetLayout);
