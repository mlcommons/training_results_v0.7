...
DEVICE_FUNCTION void parallel_sums_16x2(float *smem, float (&x)[4], int nhw,
                                        void* params_my_data, void* params_pair_data, int off,
                                        const int magic, void* params_pair_data2,
    ...
    if (warp_id == 0) {
        ...
        for (int sync_iter=0; sync_iter < sync_iters; ++sync_iter) {
            ...
            const int data_offset = xbuf_offset + flags_total
                                    + off*ELEMENTS_PER_LDG*THREADS_PER_PIXEL
                                    + ELEMENTS_PER_LDG*threadIdx.x;
            ...
            if (blockIdx.x == 0) {
                ...
                volatile int32_t * write_flag =
                    &((reinterpret_cast<int32_t*>(params_pair_data))[flags_offset]);
            ...
            volatile int32_t * read_flag =
                &((reinterpret_cast<int32_t*>(params_my_data))[flags_offset]);
            ...
            asm volatile ("ld.global.cv.v4.f32 {%0, %1, %2, %3}, [%4];" 
                : "=f"(other[0]), "=f"(other[1]), "=f"(other[2]), "=f"(other[3]) : "l"(read_data));
...
struct ParallelSums<16, 4> {
    ...
    DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[4], int nhw,
                                   void* params_my_data, void* params_pair_data,
                                   int off, const int magic,
                                   void* params_pair_data2, const int sync_iters) {
        parallel_sums_16x2<THREADS_PER_CTA>(smem, x, nhw, params_my_data, params_pair_data,
                                            off, magic, params_pair_data2, sync_iters);
    }
};
...
__global__ __launch_bounds__(THREADS_PER_CTA, DESIRED_OCCUPANCY)
    ...
    const int PIXELS_PER_THREAD_IN_SMEM = params.pixels_per_thread_in_smem;
    ...
    for (int c_blk_index = blockIdx.y; c_blk_index < params.c_blks; c_blk_index += gridDim.y) {
        ...
        if (params.sync_iters > 0) {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<THREADS_PER_CTA>(
                ...
                params.magic, params.pair_data2, params.sync_iters);
        } else {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<THREADS_PER_CTA>(
                smem, m1, thread_in_cta_nhw);
        }
