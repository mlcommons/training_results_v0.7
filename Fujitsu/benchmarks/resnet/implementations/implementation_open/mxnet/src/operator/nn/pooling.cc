...
static mshadow::LayoutFlag PoolChangeLayout(nnvm::NodeAttrs* attrs,
  ...
  outTransposes->resize(1);
  // fill rest of edges with empty axes
  auto n_inps = attrs->op->num_inputs;
  inpTransposes->resize(n_inps);
  auto n_outs = attrs->op->num_outputs;
  outTransposes->resize(n_outs);
  // set propoer layout;
  mshadow::LayoutFlag sourceLayout = mshadow::layoutFlag(attrs->dict["layout"]);
  if (targetLayout == mshadow::kUNKNOWN)
    targetLayout = common::Transpose(sourceLayout, inpTransposes->at(0));
  else
    CHECK_EQ(targetLayout, common::Transpose(sourceLayout, inpTransposes->at(0)));
  if (targetLayout == mshadow::kUNKNOWN) {
    inpTransposes->clear();
    outTransposes->clear();
    return mshadow::kUNKNOWN;
