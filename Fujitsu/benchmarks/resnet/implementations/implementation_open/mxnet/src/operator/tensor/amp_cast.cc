...
static mshadow::LayoutFlag MCastChangeLayout(nnvm::NodeAttrs* attrs,
                                             const mshadow::LayoutFlag targetLayout,
  ...
  for (size_t i = 0; i < outTransposes->size(); i++) {
    if (inpTransposes->at(i).ndim() > 0 && outTransposes->at(i).ndim() > 0) {
      bool accept = inpTransposes->at(i).ndim() == outTransposes->at(i).ndim();
      auto ndim = std::min(inpTransposes->at(i).ndim(), outTransposes->at(i).ndim());
      for (size_t j = 0; j < ndim; j++)
        accept *= inpTransposes->at(i)[j] == common::ReverseTransposeAxes(outTransposes->at(i))[j];
      if (!accept) {
        inpTransposes->clear();
        outTransposes->clear();
        return mshadow::kUNKNOWN;
      }
    } else if (inpTransposes->at(i).ndim() > 0) {
      outTransposes->at(i) = common::ReverseTransposeAxes(inpTransposes->at(i));
    } else if (outTransposes->at(i).ndim() > 0) {
      inpTransposes->at(i) = common::ReverseTransposeAxes(outTransposes->at(i));
    } else {
      continue;
    }
    valid = i;
  ...
  return common::Transpose(targetLayout, outTransposes->at(valid));
}
