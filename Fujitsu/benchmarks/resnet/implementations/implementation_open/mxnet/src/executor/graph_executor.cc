...
GraphExecutor::GraphExecutor(const nnvm::Symbol& symbol) {
  ...
  symbol_ = symbol.Copy();
  prev_cac_stop_layer_num_ = -1;
  prev_cac_stop_layer_num_update_ = -1;
}
...
void GraphExecutor::Backward(const std::vector<NDArray>& head_grads, bool is_train,
                             const GradientSkipInfo *gs_info, const size_t gs_info_len,
                             const int gs_stop_layer_num, const int gs_non_stop_layer_num,
  ...
  const auto& idx = graph_.indexed_graph();
  RunOps(is_train, num_forward_nodes_, idx.num_nodes(), gs_info, gs_info_len, gs_stop_layer_num, gs_non_stop_layer_num,
...
Graph GraphExecutor::InitGraph(nnvm::Symbol symbol,
  ...
  if (!alm_params.empty()) {
    AutomaticLayoutManager alm(symbol);
    alm.run(alm_params.getTargets());
    symbol = alm.GetOptimizedSymbol();
  }
...
void GraphExecutor::InitOpSegs(int cac_stop_layer_num) {
  ...
  if (prefer_bulk_exec_train && is_training) {
    ...
    this->BulkOpSegs(num_forward_nodes_, total_num_nodes, Imperative::BulkExecMaxNodeTrainBwd(), cac_stop_layer_num);
  }
...
void GraphExecutor::BulkOpSegs(size_t from_node, size_t up_to_node, size_t segment_num_nodes_max,
                               int cac_stop_layer_num) {
  size_t topo_start = from_node;
  size_t segment_node_count = 0;
  size_t up_to_available_node = up_to_node;
  if (cac_stop_layer_num >= 0) {
    cac_stop_layer_num = std::min(static_cast<size_t>(cac_stop_layer_num), up_to_node);
    up_to_available_node = up_to_node - cac_stop_layer_num - 1;
  }
  for (size_t nid = from_node; nid < up_to_node; nid++) {
    auto &node = graph_.indexed_graph()[nid].source;
    auto &op_node = op_nodes_[nid];
    op_node.is_cac_stop_layer = (nid >= up_to_available_node) ? true : false;
    ...
    if (!can_bulk || nid == up_to_available_node - 1 || segment_node_count >= segment_num_nodes_max) {
...
void GraphExecutor::RunOps(bool is_train, size_t topo_start, size_t topo_end,
  ...
  for (size_t nid = topo_start; nid < topo_end; ++nid) {
    ...
    opnode.exec->op_ctx.is_cac_gradient_skip = false;
    if (topo_start >= num_forward_nodes_ && gs_info) {
      std::string attr_name = opnode.attr_name;
      std::string::size_type pos = attr_name.find("_backward");
      if (pos != std::string::npos) {
        std::string target_layer_name = attr_name.substr(0, pos);
        for (size_t i = 0; i < gs_info_len; ++i) {
          std::string layer_name = gs_info[i].key;
          if (layer_name.find(target_layer_name) != std::string::npos) {
            opnode.exec->op_ctx.is_cac_gradient_skip = (gs_info[i].skip_term > 0);
            break;
          }
        }
      }
  ...
  if (topo_start >= num_forward_nodes_) {
    if (gs_stop_layer_num != prev_cac_stop_layer_num_) {
      prev_cac_stop_layer_num_ = gs_stop_layer_num;
      int fitted_gs_stop_layer_num = gs_stop_layer_num;
      size_t nid = std::max(0, static_cast<int>(topo_end) - gs_stop_layer_num - 1);
      if (gs_stop_layer_num >= 0 && nid > topo_start) {
        std::string attr_name = op_nodes_[nid].attr_name;
        std::string::size_type pos = attr_name.find("_bn");
        if (pos != std::string::npos) {
          std::vector<std::string> bn_seg_delimiter = {"relu", "add_n"};
          std::string prev_attr_name = op_nodes_[nid - 1].attr_name;
          for (auto itr : bn_seg_delimiter) {
            pos = prev_attr_name.find(itr);
            if (pos != std::string::npos) {
              fitted_gs_stop_layer_num = gs_stop_layer_num + 1;
              break;
            }
          }
        }
      }
      InitOpSegs(fitted_gs_stop_layer_num);
      prev_cac_stop_layer_num_update_ = -1;
    }
    if (gs_info && gs_stop_layer_num != -1 && prev_cac_stop_layer_num_update_ == -1) {
      for (size_t nid = topo_start; nid < topo_end; ++nid) {
        OpNode& opnode = op_nodes_[nid];
        if (opnode.is_cac_stop_layer) {
          std::string attr_name = opnode.attr_name;
          std::string stop_layer_name = attr_name.substr(0, attr_name.find("_backward"));
          *gs_stop_layer_num_update = prev_cac_stop_layer_num_update_ = gs_stop_layer_num;
          for (int i = static_cast<int>(gs_info_len) - 1; i >= 0; --i) {
            std::string layer_name = gs_info[i].key;
            if (layer_name.find(stop_layer_name) != std::string::npos) {
              if (gs_stop_layer_num_update) {
                *gs_stop_layer_num_update = prev_cac_stop_layer_num_update_ = gs_info[i].layer_num;
                break;
              }
            }
          }
    ...
    } else if (gs_stop_layer_num_update) {
      *gs_stop_layer_num_update = prev_cac_stop_layer_num_update_;
    }
  }
  ...
  for (size_t nid = topo_start; nid < topo_end; ++nid) {
    ...
    if (op_nodes_[nid].is_cac_stop_layer) {
      if (gs_non_stop_layer_num == -1 || nid < std::max(0, static_cast<int>(topo_end) - gs_non_stop_layer_num - 1)) {
        continue;
      }
    }
    // Monitor callbacks
