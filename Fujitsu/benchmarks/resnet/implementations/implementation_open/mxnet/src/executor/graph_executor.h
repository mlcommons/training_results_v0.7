...
class GraphExecutor : public Executor {
 public:
  ...
  void Backward(const std::vector<NDArray> &head_grads, bool is_train = true,
                const GradientSkipInfo *gs_info = nullptr, const size_t gs_info_len = 0,
                const int gs_stop_layer_num = -1, const int gs_non_stop_layer_num = -1,
                int *gs_stop_layer_num_update = nullptr) override;
 ...
 protected:
  ...
  struct OpNode {
    ...
    // cac-stop-layer of this node
    bool is_cac_stop_layer{false};
  ...
  void InitOpSegs(int cac_stop_layer_num = -1);
  ...
  */
  ...
  void BulkOpSegs(size_t from_node, size_t up_to_node, size_t segment_num_nodes_max, int cac_stop_layer_num = -1);
  ...
  // child executors
  nnvm::Symbol symbol_;
  // previous cac_stop_layer_num
  int prev_cac_stop_layer_num_;
  // previous cac_stop_layer_num for update
  int prev_cac_stop_layer_num_update_;
};
