...
Graph DetectInplaceAddTo(Graph g) {
  ...
  static bool fix_addto = dmlc::GetEnv("MXNET_EXEC_ENABLE_ADDTO", false);
  ...
  for (uint32_t nid = 0; nid < idx.num_nodes(); ++nid) {
    ...
    bool is_grad_summation_op = inode.source->op() == ewise_plus_op;
    if (fix_addto) {
      is_grad_summation_op = is_grad_summation_op ||
                             ((inode.source->op() == ewise_sum_op) &&
                              (inode.source->attrs.name == "sum_grad"));
    }
    if (!is_grad_summation_op) continue;
    if (inode.inputs.size() != 2) continue;
