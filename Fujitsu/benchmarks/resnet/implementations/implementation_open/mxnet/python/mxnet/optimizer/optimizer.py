...
class Optimizer(object):
    ...
    def __init__(self, rescale_grad=1., param_idx2name=None, wd=0.,
        ...
        self.total_skip_num = {}
        self.my_segment = {}
        self.cac_gradient_skip = int(os.getenv('CAC_GRADIENT_SKIP','0'))
        if os.getenv('CAC_STOP_LAYER_NUM',"") != "":
            self.cac_gradient_skip = 0
        self.cac_init_skip_time = [int(num) for num in (os.getenv('CAC_INIT_SKIP_TIME','0')).split(',')]
        self.cac_init_grace_time = [int(num) for num in (os.getenv('CAC_INIT_GRACE_TIME','0')).split(',')]
        self.cac_skip_time = [int(num) for num in (os.getenv('CAC_SKIP_TIME','1')).split(',')]
        self.cac_grace_time = [int(num) for num in (os.getenv('CAC_GRACE_TIME','4')).split(',')]
        self.segment = []
        self.target =  []
        self.non_target =  []
        if os.getenv('CAC_SKIP_TARGET_LAYER',"") != "":
            self.segment = self._text2array(os.getenv('CAC_SKIP_TARGET_LAYER',""))
            for a in self.segment:
                self.target.extend(a)
        if os.getenv('CAC_NON_SKIP_TARGET_LAYER',"") != "":
            tmp = self._text2array(os.getenv('CAC_NON_SKIP_TARGET_LAYER',""))
            for a in tmp:
                self.non_target.extend(a)
        self.prev_grad = {}
        self.naive_skip_flag = int(os.getenv('CAC_NAIVE_SKIP',"-1"))
        self.freq_flag = int(os.getenv('CAC_FREQ_SCALE',"-1"))
        self.gdif_val = float(os.getenv('CAC_GDIF_THRESH',"-1"))
        self.gwm_val = float(os.getenv('CAC_GWM_THRESH',"-1"))
    opt_registry = {}
    ...
    def _text2array(self, text):
        ...
        for target in segments:
            ...
            for n in nums:
                if "-" in n:
                    serial = [int(a) for a in n.split('-')]
                    tmp = range(serial[0],serial[1]+1)
                    num_list.extend(tmp)
                else:
                    num_list.extend([int(n)])
            out.append(num_list)
        return out

    def _is_skip(self, idx):
        if idx in self.skip_count:
            self.skip_term[idx] = self.skip_count[idx]
            if self.skip_count[idx] > 0:
                self.skip_count[idx] -= 1
                if self.skip_count[idx] == 0:
                    self.grace_count[idx] = self.cac_grace_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_grace_time) else 0)]
                return True
            else:
                return False
        else:
    ...
    def _skip_judge(self, w, g, idx, lr, wd, st, w_norm=None, g_norm=None):
        if idx in self.non_target:
            return False
        if idx in self.target or len(self.target) == 0:
            grad2 = 0.0

            if self.gdif_val >= 0 or self.gwm_val >= 0:
                if g_norm is None:
                    grad2 = self._l2norm(g)
                else:
                    grad2 = g_norm.asscalar()
                    #print(idx,"grad2",'{:.6g}'.format(grad2),'{:.6g}'.format(self._l2norm(g)))
                    assert math.isclose(grad2, self._l2norm(g), rel_tol=1e-6), 'l2 norm of grad does not match'

            # grad_diff skip
            if self.gdif_val >= 0:
                grad_diff = abs(self.prev_grad[idx] - grad2 * self.rescale_grad)
                self.prev_grad[idx] = grad2 * self.rescale_grad
                #print(idx,"grad_diff",grad_diff,self.gdif_val)
                if grad_diff < self.gdif_val:
                    return True

            # gwm skip
            if self.gwm_val >= 0:
                if w_norm is None:
                    weight2 = self._l2norm(w)
                else:
                    weight2 = w_norm.asscalar()
                    #print(idx,"weight2",'{:.6g}'.format(weight2),'{:.6g}'.format(self._l2norm(w)))
                    assert math.isclose(weight2, self._l2norm(w), rel_tol=1e-6), 'l2 norm of weight does not match'
                mom2 = self._l2norm(st[0])
                try:
                    if weight2 > 0.0 and grad2 > 0.0:
                        lars = self.lars_eta * weight2/(grad2 + wd * weight2 + self.lars_eps)
                    else:
                        lars = 1.0
                except NameError :
                    lars = 1.0
                actual_energy = grad2 * lr * lars * self.rescale_grad + lr * lars * wd * weight2 + self.momentum * mom2
                #print(idx,"gwm_val",actual_energy,self.gwm_val)
                if actual_energy < self.gwm_val:
                    return True

            # Naive skip
            if self.naive_skip_flag > 0:
                if self.skip_count[idx] == 0:
                    return True
        return False

    def freq_scale(self, g, idx):
        if idx in self.non_target:
            return
        if idx in self.target or len(self.target) == 0:
            tmp_factor = 1.0/(1.0-(self.cac_skip_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_skip_time) else 0)]/(self.cac_skip_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_skip_time) else 0)] + self.cac_grace_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_grace_time) else 0)])))
            g.__imul__(tmp_factor)
            #print(idx,"freq_scale")
        return

    def gradient_skip(self, w, g, idx, lr, wd, st, w_norm=None, g_norm=None):
        # Initialize
        self.skip_term[idx] = 0
        if not idx in self.skip_count:
            self.skip_count[idx] = 0
            self.grace_count[idx] = 0
            self.total_skip_num[idx] = 0
            self.prev_grad[idx] = 0.0
            self.my_segment[idx] = 0
            for i, seg in enumerate(self.segment):
                if idx in seg:
                    self.my_segment[idx] = i
                    self.skip_count[idx] = self.cac_init_skip_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_init_skip_time) else 0)]
                    self.grace_count[idx] = self.cac_init_grace_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_init_grace_time) else 0)]
                    #print("Idx:"+str(idx)+" Initizalize to " + str(self.grace_count[idx]))
                #print("initialize")
        # Grace time
        if self.grace_count[idx] > 0:
            self.grace_count[idx] -= 1
            ##############################
            if self.freq_flag > 0:
                self.freq_scale(g,idx)
            ##############################
            #print("No skip, [" + str(self.grace_count[idx]) +"/" + str(self.cac_grace_time) + "]")
            if not self.grace_count[idx] == 0:
                return False
        # Skip process
        if self._is_skip(idx):
            #g.__imul__(0.0)
            #print("Skipping, [" + str(self.skip_count[idx]) +"/" + str(self.cac_skip_time) + "]")
            return True
        # Determination process
        if self._skip_judge(w, g, idx, lr, wd, st, w_norm, g_norm):
            for i, seg in enumerate(self.segment):
                self.skip_count[idx] = self.cac_skip_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_skip_time) else 0)]
                self.total_skip_num[idx] += self.cac_skip_time[(self.my_segment[idx] if self.my_segment[idx] < len(self.cac_skip_time) else 0)]
            #print("Skip start")
            return False
        return False
    ...
    def _update_impl(self, indices, weights, grads, states, multi_precision=False):
        ...
        if aggregate:
            ...
            for weight, grad, state, lr, wd in zip(weights, grads, states, lrs, wds):
                if not multi_precision:
                    if state is not None:
                        if not self.gradient_skip(weight, grad, i, lr, wd, state):
                            sgd_mom_update(weight, grad, state, out=weight,
                                           lazy_update=self.lazy_update, lr=lr, wd=wd, **kwargs)
                    else:
                        sgd_update(weight, grad, out=weight, lazy_update=self.lazy_update,
