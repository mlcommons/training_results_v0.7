...
def _update_params_on_kvstore(param_arrays, grad_arrays, kvstore, param_names, cac_gradient_skip_dict=None, cac_stop_layer_num=-1):
    ...
    for index, pair in enumerate(zip(param_arrays, grad_arrays)):
        ...
        if not cac_gradient_skip_dict is None:
            if 'conv' in name and cac_gradient_skip_dict[name][0] is True:
                continue
        # push gradient, priority is negative index
...
def _update_params(param_arrays, grad_arrays, updater, num_device,
                   kvstore=None, param_names=None, cac_gradient_skip_dict=None, cac_stop_layer_num=-1):
    ...
    for i, pair in enumerate(zip(param_arrays, grad_arrays)):
        ...
        if grad_list[0] is None:
            continue
        index = i
        if index <= cac_stop_layer_num:
            continue
        if kvstore:
            name = param_names[index]
            if not cac_gradient_skip_dict is None:
                if name in cac_gradient_skip_dict:
                    if cac_gradient_skip_dict[name][0] > 0:
                        continue
            # push gradient, priority is negative index
            kvstore.push(name, grad_list, priority=-index)
    ...
    if not cac_gradient_skip_dict is None:
        for index in range(len(param_names)):
            name = param_names[index]
            if len(cac_gradient_skip_dict[name]) == 0:
                cac_gradient_skip_dict[name].extend([0, 0])
            skip_term = updater.optimizer.skip_term[index] if index in updater.optimizer.skip_term else 0
            cac_gradient_skip_dict[name] = [skip_term, index]
