...
class GradientSkipInfo(ctypes.Structure):
    _fields_ = [('key', ctypes.c_char_p), ('skip_term', ctypes.c_int), ('layer_num', ctypes.c_int)]
...
class Executor(object):
    ...
    def backward(self, out_grads=None, is_train=True, cac_gradient_skip_dict=None, cac_stop_layer_num=-1,
        ...
        if cac_gradient_skip_dict is None and cac_stop_layer_num == -1:
            ...
            if not cac_gradient_skip_dict is None:
                gs_list = []
                for k,v in cac_gradient_skip_dict.items():
                    gs = GradientSkipInfo()
                    gs.key = k.encode('utf8')
                    gs.skip_term = (ctypes.c_int)(v[0])
                    gs.layer_num = (ctypes.c_int)(v[1])
                    gs_list.append(gs)
                gs_array = (GradientSkipInfo * len(gs_list))(*gs_list)
                gs_array_len = len(gs_array)
            stop_layer_num_update = ctypes.c_int()
            check_call(_LIB.MXExecutorBackwardExWithGradientSkip(
                self.handle,
                mx_uint(len(out_grads)),
                ndarray,
                ctypes.c_int(is_train),
                gs_array,
                gs_array_len,
                ctypes.c_int(cac_stop_layer_num),
                ctypes.c_int(cac_non_stop_layer_num),
                ctypes.byref(stop_layer_num_update)))
            cac_gradient_skip_dict['cac_stop_layer_num_update'] = [stop_layer_num_update.value, 0]
