...
from mxnet.ndarray import (NDArray, zeros, clip, sqrt, cast, maximum, abs as NDabs, array, multiply,
                           multi_sum_sq, multi_lars, multi_lars_norms)
    ...
    def _update_impl(self, indices, weights, grads, states, multi_precision=False):
        ...
        if aggregate:
            ...
            if self.lars and len(lars_idx) > 0:
                ...
                new_lrs_norms = array([[0 for i in full_idx] for j in range(3)], ctx=cur_ctx, dtype='float32')
                if self.cac_gradient_skip == 0:
                    multi_lars(new_lrs[:nb_lars], w_sum_sq, g_sum_sq, new_wds[:nb_lars],
                               ...
                               out=new_lrs_norms[0][:nb_lars])
                else:
                    new_lrs_norms = multi_lars_norms(new_lrs[:nb_lars], w_sum_sq, g_sum_sq, new_wds[:nb_lars],
                                                     eta=self.lars_eta, eps=self.lars_eps,
                ...
                while sidx < len(indices):
                    ...
                    if self.cac_gradient_skip == 0:
                        ...
                        is_skips = True
                        eidx = min(eidx, len(indices)-1)
                        for idx in range(sidx, min(eidx, nb_lars)):
                            is_skip = self.gradient_skip(weights[idx], grads[idx], indices[idx], lrs[idx], wds[idx],
                                                         states[idx], new_lrs_norms[1][idx], new_lrs_norms[2][idx])
                            is_skips = is_skips and is_skip
                    if not is_skips:
                        if not multi_precision:
                            if self.momentum > 0:
                ...
                while current_index < len(indices):
                    ...
                    if self.cac_gradient_skip == 0:
                        ...
                        is_skips = True
                        for idx in range(sidx, min(eidx, len(indices))):
                            # TODO: Reuse L2 Norm from _get_lars()
                            is_skip = self.gradient_skip(weights[idx], grads[idx], idx, lrs[idx], wds[idx], states[idx])
                            is_skips = is_skips and is_skip
                    if not is_skips:
                        if not multi_precision:
                            if self.momentum > 0:
            ...
            for i, weight, grad, state, lr, wd in zip(indices, weights, grads, states, lrs, wds):
                if not multi_precision:
                    if state is not None:
                        # TODO: Reuse L2 Norm from _get_lars()
                        if self.cac_gradient_skip == 0 or not self.gradient_skip(weight, grad, i, lr, wd, state):
                            sgd_mom_update(weight, grad, state, out=weight,
                                        lazy_update=self.lazy_update, lr=lr, wd=wd, **kwargs)
                    else:
                        sgd_update(weight, grad, out=weight, lazy_update=self.lazy_update,
...
def add_fit_args(parser):
    ...
    train.add_argument('--cac-gradient-skip', type=int, default='0', help='enable cac-gradient-skip')
    return train
...
def mlperf_fit(self, args, train_data, eval_data=None, eval_metric='acc',
    ...
    # training loop
    ################################################################################
    cac_gradient_skip_dict = None
    cac_stop_layer_nums = []
    cac_stop_layer_change_epoch_nums = []
    if args.cac_gradient_skip > 0:
        print('enable cac-gradient-skip.')
        cac_gradient_skip_dict = defaultdict(list)
        cac_gradient_skip_dict['cac_stop_layer_num_update'].extend([-1, 0])
        ...
        cac_non_stop_layer_num = int(os.getenv('CAC_NON_STOP_LAYER_NUM','-1'))
    for epoch in range(begin_epoch, num_epoch):
        ...
        while not end_of_batch:
            ...
            if monitor is not None:
                monitor.tic()
            self.forward_backward(data_batch, cac_gradient_skip_dict, cac_stop_layer_num, cac_non_stop_layer_num)
            self.update(cac_gradient_skip_dict, cac_gradient_skip_dict['cac_stop_layer_num_update'][0])
