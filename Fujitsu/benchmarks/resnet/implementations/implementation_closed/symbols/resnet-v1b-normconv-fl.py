...
def residual_unit_norm_conv(data, nchw_inshape, num_filter, stride, dim_match, name, bottle_neck=True,
    ...
    if bottle_neck:
        shape = resnet_begin_block_log(shape, "BOTTLENECK_BLOCK")
        ...
        shape = resnet_conv2d_log(shape, 1, int(num_filter*0.25), "TRUNCATED_NORMAL", False)
        ...
        shape = resnet_conv2d_log(shape, stride, int(num_filter*0.25), "TRUNCATED_NORMAL", False)
        ...
        shape = resnet_relu_log(shape)
        shape = resnet_conv2d_log(shape, 1, int(num_filter), "TRUNCATED_NORMAL", False)
        ...
        if dim_match:
            ...
            proj_shape = resnet_conv2d_log(nchw_inshape[1:], stride, int(num_filter), "TRUNCATED_NORMAL", False)
...
#    mx_resnet_print(key=mlperf_log.MODEL_HP_INITIAL_SHAPE, val=shape)
    ...
    shape = resnet_conv2d_log(shape, 2, filter_list[0], "TRUNCATED_NORMAL", False)
