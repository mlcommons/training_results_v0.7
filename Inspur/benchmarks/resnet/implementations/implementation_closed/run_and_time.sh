...
SLURM_NTASKS_PER_NODE=${SLURM_NTASKS_PER_NODE:-$NFNGPU}
...
if [ -n "${SLURM_LOCALID-}" ]; then
  ...
  DISTRIBUTED="mpirun --allow-run-as-root --bind-to none --np ${NFNGPU}"
fi
...
if [[ "${KVSTORE}" == "horovod" ]]; then
    ...
    #BIND="./bind.sh --cpu=nf5488_cxx.sh --mem=nf5488_cxx.sh --cluster=${cluster} --"
