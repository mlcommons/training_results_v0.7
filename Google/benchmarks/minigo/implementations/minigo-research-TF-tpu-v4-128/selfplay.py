...
# Lint as: python3
# pylint: disable=g-doc-args, g-doc-return-or-yield,superfluous-parens
...
from REDACTED.minigo.strategies import MCTSPlayer
...
def play(network):
  ...
  while True:
    ...
    if (FLAGS.verbose >= 2) or (FLAGS.verbose >= 1 and
                                player.root.position.n % 10 == 9):
      print('Q: {:.5f}'.format(player.root.Q))
      dur = time.time() - start
      print('%d: %d readouts, %.3f s/100. (%.2f sec)' %
            (player.root.position.n, readouts, dur / readouts * 100.0, dur))
    if FLAGS.verbose >= 3:
      print('Played >>', coords.to_gtp(coords.from_flat(player.root.fmove)))
...
def run_game(load_file, selfplay_dir=None, holdout_dir=None,
  ...
  if selfplay_dir is not None:
    ...
    if random.random() < holdout_pct:
      ...
      fname = os.path.join(selfplay_dir, '{}.tfrecord.zz'.format(output_name))
