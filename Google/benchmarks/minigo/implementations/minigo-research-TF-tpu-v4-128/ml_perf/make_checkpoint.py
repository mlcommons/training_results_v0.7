...
# pylint: disable=superfluous-parens,missing-docstring,unused-import
...
def find_model_num():
  ...
  print('Looking for selfplay data directories that match "{}"'.format(pattern))
  paths = sorted(tf.io.gfile.glob(pattern))
  if not paths:
    raise RuntimeError('Couldn\'t find selfplay data for checkpoint {}'.format(
        FLAGS.ckpt))
  ...
  while model_num > 0:
    ...
    pattern = os.path.join(FLAGS.selfplay_dir, model_name, '*', '*',
                           '*.tfrecord.zz')
    num_older = len([
        x for x in tf.io.gfile.glob(pattern)
...
def copy_flags():
  utils.copy_tree(FLAGS.flag_dir, os.path.join(FLAGS.checkpoint_dir, 'flags'))
...
def copy_selfplay_data(checkpoint_model_num):
  ...
  paths = tf.io.gfile.glob(
      os.path.join(FLAGS.selfplay_dir, get_model_name(latest_model_num), '*',
                   '*', '*.tfrecord.zz'))
