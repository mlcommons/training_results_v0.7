...
# pylint: disable=bad-indentation,unbalanced-tuple-unpacking,superfluous-parens,g-builtin-op,anomalous-backslash-in-string,old-style-class,missing-docstring
...
def evaluate_model(eval_model_path):
  ...
  for i, device in enumerate(FLAGS.devices):
    ...
    processes.append(
        utils.checked_run([
            'bazel-bin/cc/eval', '--flagfile={}'.format(
                os.path.join(FLAGS.flags_dir, 'eval.flags')),
            '--eval_model={}'.format(eval_model_path),
            '--target_model={}'.format(FLAGS.target), '--sgf_dir={}'.format(
                FLAGS.sgf_dir), '--parallel_games={}'.format(num_games),
            '--verbose=false'
        ], env))
  all_output = utils.wait(processes)
...
def main(unused_argv):
  ...
  for _, (timestamp, name, path) in enumerate(models):
