...
# pylint: disable=superfluous-parens,missing-docstring,g-complex-comprehension,unused-variable
...
def training_series(cursor_r, cursor_c, mix, increment_fraction=0.05):
  ...
  print('stride_c was {}, now {}'.format(mix.games_c * increment_fraction,
                                         stride_c))
  print('stride_r: {}  stride_c: {}'.format(stride_r, stride_c))
  for b_r, b_c in zip(
      range(0, cursor_r, stride_r), range(0, cursor_c, stride_c)):
...
def _export_training_set(args):
  ...
  with tf.Session() as sess:
    ds = bigtable_input.get_unparsed_moves_from_games(gq_r, gq_c, start_r,
                                                      start_c, mix)
    ...
    with tf.io.TFRecordWriter(
      ...
      with open(log_filename, 'w') as progress_file:
        with tqdm(
            desc='Records', unit_scale=2, total=total_moves,
...
def main(argv):
  ...
  bounds = list(
      training_series(gq_r.latest_game_number, gq_c.latest_game_number, mix,
                      fresh))
  trainings = [(spec, start_r, start_c, mix, batch_size,
                '{}{:0>10}_{:0>10}.tfrecord.zz'.format(output_prefix, start_r,
                                                       start_c))
               for start_r, finish_r, start_c, finish_c in bounds]
