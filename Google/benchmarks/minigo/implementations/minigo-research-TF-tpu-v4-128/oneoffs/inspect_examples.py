...
# pylint: disable=unused-argument,broad-except,invalid-name,bare-except,superfluous-parens,missing-docstring,g-doc-args,g-complex-comprehension,unused-variable
...
def guess_format(x, pi, num_records):
  ...
  for board_size in [9, 19]:
    for feature_layout in ['nhwc', 'nchw']:
      try:
        to_play = guess_to_play(x, pi, num_records, board_size, feature_layout)
...
def read_examples(path):
  ...
  if not (FLAGS.board_size and FLAGS.feature_layout and FLAGS.to_play_feature):
    try:
...
def print_example(examples, i):
  ...
  pi_lines.append(
      format_pi(example.pi[-1], go.EMPTY, mean, mx, example.c == go.N * go.N))
