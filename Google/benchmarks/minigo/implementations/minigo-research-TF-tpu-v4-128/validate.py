...
# pylint: disable=missing-docstring,g-doc-return-or-yield,dangerous-default-value,g-doc-args
...
flags.DEFINE_string('validate_data_path', '', 'Path to validation set.')
    ...
    with utils.logged_timer('Building lists of holdout files'):
      ...
      while dirs:
        ...
        for path, newdirs, files in os.walk(d):
          tf_records.extend(
              os.path.join(path, f) for f in files if f.endswith('.zz'))
