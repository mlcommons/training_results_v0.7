...
# pylint: disable=g-complex-comprehension,unidiomatic-typecheck,redefined-outer-name,dangerous-default-value,old-style-class,invalid-name,g-docstring-quotes,g-doc-return-or-yield,g-short-docstring-punctuation,g-no-space-after-docstring-summary,missing-docstring,g-space-before-docstring-summary,g-doc-args,g-importing-member,g-bad-exception-name
...
(0, 0) is considered to be the upper left corner of the board, and (18, 0) is
the lower left.
"""
...
class Group(namedtuple('Group', ['id', 'stones', 'liberties', 'color'])):
  """
    ...
    liberties: a frozenset of Coordinates that are empty and adjacent to this
    group.
...
class LibertyTracker():
  ...
  def add_stone(self, color, c):
    ...
    new_group = self._merge_from_played(color, c, empty_neighbors,
    ...
    # suicide is illegal
    if not self.groups[new_group.id].liberties:
  ...
  def _merge_from_played(self, color, played, libs, other_group_ids):
    ...
    self.max_group_id += 1
    result = Group(self.max_group_id, frozenset(stones), frozenset(liberties),
                   color)
...
class Position():
  ...
  def __init__(self,
               ...
               board_deltas=None,
               to_play=BLACK):
    """Initialization for Position class.
    ...
    Args:
        ...
        board_deltas: a np.array of shape (n, go.N, go.N) representing changes
            ...
            Should satisfy next_pos.board - next_pos.board_deltas[0] ==
            pos.board
  ...
  def __str__(self, colors=True):
    ...
    for i in range(N):
      row = [' ']
      for j in range(N):
        appended = '<' if (self.recent and
                           (i, j) == self.recent[-1].move) else ' '
    ...
    annotated_board = '\n'.join(
        itertools.chain(header_footer_rows, annotated_board_contents,
                        header_footer_rows))
  ...
  def all_legal_moves(self):
    ...
    num_adjacent_stones = (
        adjacent[:-2, 1:-1] + adjacent[1:-1, :-2] + adjacent[2:, 1:-1] +
        adjacent[1:-1, 2:])
    # Surrounded spots are those that are empty and have 4 adjacent stones.
    surrounded_spots = np.multiply((self.board == EMPTY),
  ...
  def play_move(self, c, color=None, mutate=False):
    ...
    if not self.is_move_legal(c):
      raise IllegalMove('{} move at {} is illegal: \n{}'.format(
          'Black' if self.to_play == BLACK else 'White', coords.to_gtp(c),
          self))
  ...
  def is_game_over(self):
    return (len(self.recent) >= 2 and self.recent[-1].move is None and
