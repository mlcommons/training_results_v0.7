...
# pylint: disable=g-complex-comprehension,g-importing-member,missing-super-argument,syntax-error,g-ambiguous-str-annotation, missing-docstring,g-ambiguous-str-annotation, unused-argument, redefined-outer-name,syntax-error,g-ambiguous-str-annotation, bare-except
...
# pylint: disable=g-ambiguous-str-annotation, bare-except
class BasicCmdHandler(object):
  ...
  def cmd_boardsize(self, n):
    if n != go.N:
      raise ValueError("unsupported board size: {}".format(n))

  def cmd_clear_board(self):
    position = self._player.get_position()
    if (self._player.get_result_string() and position and
        len(position.recent) > 1):
      try:
        sgf = self._player.to_sgf()
        with open(datetime.datetime.now().strftime("%Y-%m-%d-%H:%M.sgf"),
                  "w") as f:
          f.write(sgf)
  ...
  def cmd_play(self, arg0, arg1=None):
    if arg1 is None:
      move = arg0
    else:
  ...
  def cmd_genmove(self, color=None):
    ...
    if self._player.should_resign():
      self._player.set_result(
          -1 * self._player.get_position().to_play, was_resign=True)
      return "resign"

    self._player.play_move(move)
    if self._player.get_root().is_done():
      self._player.set_result(
          self._player.get_position().result(), was_resign=False)
  ...
  def _accomodate_out_of_turn(self, color):
    position = self._player.get_position()
...
class GoGuiCmdHandler(object):
  ...
  def cmd_gogui_analyze_commands(self):
    return "\n".join([
        "var/Most Read Variation/nextplay", "var/Think a spell/spin",
        "var/Final score/final_score", "pspairs/Visit Heatmap/visit_heatmap",
...
class MiniguiBasicCmdHandler(BasicCmdHandler):
  ...
  def cmd_info(self):
    return ("num_readouts: %d report_search_interval: %.1f n: %d "
            "resign_threshold: %f" %
            (self._player.get_num_readouts(), self._report_search_interval *
             1000, go.N, self._player.resign_threshold))

  def cmd_readouts(self, readouts):
    readouts = max(8, readouts)
    self._player.set_num_readouts(readouts)
    return readouts

  def cmd_report_search_interval(self, interval_ms):
  ...
  def cmd_play(self, arg0, arg1=None):
    ...
    root = self._player.get_root()
    if root.is_done():
      self._player.set_result(root.position.result(), was_resign=False)
