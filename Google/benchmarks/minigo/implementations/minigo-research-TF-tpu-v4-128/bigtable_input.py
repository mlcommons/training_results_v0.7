...
#
# from REDACTED.minigo import utils # pylint: disable=line-too-long
#
#
...
#                     'The identifier of the cloud bigtable instance in cbt_project') # pylint: disable=line-too-long
#
...
# #
# # https://github.com/googleapis/google-cloud-go/blob/master/bigtable/cmd/loadtest/loadtest.go # pylint: disable=line-too-long
#
...
#     """
#   if isinstance(ds.output_types, tuple) or isinstance(ds.output_shapes, tuple): # pylint: disable=line-too-long
...
#   ds = ds.map(lambda x: tf.strings.to_number(
#       tf.strings.substr(x, 2, 10) + '.' + tf.strings.substr(x, 15, 3),
#       out_type=tf.float64))
...
#         TABLE_STATE,
#         filter_=bigtable_row_filters.ColumnRangeFilter(METADATA, GAME_COUNTER,
#                                                        GAME_COUNTER))
...
#         filter_=bigtable_row_filters.ColumnRangeFilter(METADATA, move_count,
#                                                        move_count))
#
#     def parse(r):
...
#
#         Args:
#           format_str:  a string to `.format()` by the game numbers in order to
#             create the row prefixes.
...
#         filter_=bigtable_row_filters.ColumnRangeFilter(METADATA, bleak, bleak)) # pylint: disable=line-too-long
#
#     def parse(r):
#       rk = str(r.row_key, 'utf-8')
...
#         Increments the cell `table_state=metadata:wait_for_game_number` by the
#           given number of games.  This will cause `self.wait_for_fresh_games()` # pylint: disable=line-too-long
#           to block until the game counter has reached this number.
#     """
#     latest = self.latest_game_number
#     table_state = self.bt_table.row(TABLE_STATE)
...
#         If the cell `table_state=metadata:wait_for_game_number` exists, then
#           block the caller, checking every `poll_interval` seconds,
#         until `table_state=metadata:game_counter is at least the value in that
#           cell.
#     """
...
#                     latest_game, wait_until_game, latest_game - last_latest,
...
#     table_state = self.bt_table.read_row(
#         TABLE_STATE,
#         filter_=bigtable_row_filters.ColumnRangeFilter(METADATA, WAIT_CELL,
#                                                        WAIT_CELL))
#     if table_state is None:
...
#           game_end:  integer, ending game  Uses the `ct_` keyspace for rapid
#             move summary.
#     """
#     rows = self.bt_table.read_rows(
...
#           moves:  an integer indicating how many moves should be sampled from
#             those N games.
...
#     utils.dbg(
#         'Row range: %s - %s; total moves: %d; probability %.3f; moves %d' %
#         (start_row, end_row, total_moves, probability, moves))
#     ds = self.tf_table.parallel_scan_range(
#         start_row,
#         end_row,
...
#     wr_op = self.tf_table.write(
#         mc, column_families=[METADATA], columns=[MOVE_COUNT])
#     sess.run(wr_op)
#
...
#   """Sets the metadata cell used to block until some quantity of games have been played. # pylint: disable=line-too-long
#
#     This sets the 'freshness mark' on the `game_queue`, used to block training
#     until enough new games have been played.  The number of fresh games required # pylint: disable=line-too-long
#     is the larger of:
#        - The fraction of the total window size
#        - The `minimum_fresh` parameter
#     The number of games required can be indexed from the 'count_from' parameter. # pylint: disable=line-too-long
#     Args:
...
#       fresh_fraction: a float in (0,1] indicating the fraction of games to wait # pylint: disable=line-too-long
#         for
#       minimum_fresh:  an integer indicating the lower bound on the number of new # pylint: disable=line-too-long
#         games.
#   """
#   already_played = game_queue.latest_game_number - count_from
#   print('== already_played: ', already_played, flush=True)
#   if window_size > count_from:  # How to handle the case when the window is not yet 'full' # pylint: disable=line-too-long
#     game_queue.require_fresh_games(int(minimum_fresh * .9))
#   else:
#     num_to_play = max(
#         0,
#         math.ceil(window_size * .9 * fresh_fraction) - already_played)
...
#   return GameMix(games_r, moves_r, games_c, moves_c, selection)
#
#
...
#
#     Args:
#       games, games_nr: GameQueues of the regular selfplay and calibration (aka
#         'no resign') games to sample from.
#       n:  an integer indicating how many past games should be sourced.
#       moves:  an integer indicating how many moves should be sampled from those # pylint: disable=line-too-long
#         N games.
#       column_family:  name of the column family containing move examples.
#       column:  name of the column containing move examples.
#       shuffle:  if True, shuffle the selected move examples.
...
#   resign = games.moves_from_last_n_games(mix.games_r, mix.moves_r, shuffle,
#                                          column_family, column)
#   no_resign = games_nr.moves_from_last_n_games(mix.games_c, mix.moves_c,
#                                                shuffle, column_family, column)
#   choice = tf.data.Dataset.from_tensor_slices(
#       mix.selection).repeat().take(moves)
...
#
#     Args:
#       games_r, games_c: GameQueues of the regular selfplay and calibration (aka # pylint: disable=line-too-long
#         'no resign') games to sample from.
...
#     """
#   resign = games_r.moves_from_games(start_r, start_r + mix.games_r, mix.moves_r, # pylint: disable=line-too-long
#                                     shuffle, column_family, column)
#   calibrated = games_c.moves_from_games(start_c, start_c + mix.games_c,
#                                         mix.moves_c, shuffle, column_family,
#                                         column)
