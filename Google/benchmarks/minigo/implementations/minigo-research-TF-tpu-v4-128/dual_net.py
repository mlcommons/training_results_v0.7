...
# pylint: disable=missing-docstring,g-import-not-at-top,unnecessary-lambda,g-doc-return-or-yield,g-long-lambda, g-doc-args,unused-argument, redefined-outer-name, old-style-class
...
from REDACTED.learning.REDACTED.contrib.tpu_modeling.inference_converter import converter_cli
from REDACTED.learning.REDACTED.contrib.tpu_modeling.inference_converter import converter_options_pb2
from REDACTED.tensorflow.contrib import cluster_resolver as contrib_cluster_resolver
from REDACTED.tensorflow.contrib import quantize as contrib_quantize
from REDACTED.tensorflow.contrib import summary as contrib_summary
import REDACTED.tensorflow.contrib.tensorrt as trt
from REDACTED.tensorflow.contrib.tpu.python.ops import tpu_ops
from REDACTED.tensorflow.contrib.tpu.python.tpu import tpu_config as contrib_tpu_python_tpu_tpu_config
from REDACTED.tensorflow.contrib.tpu.python.tpu import tpu_estimator as contrib_tpu_python_tpu_tpu_estimator
from REDACTED.tensorflow.contrib.tpu.python.tpu import tpu_optimizer as contrib_tpu_python_tpu_tpu_optimizer
from REDACTED import rewriter_config_pb2
from REDACTED.tensorflow.python.framework import function
from REDACTED.tensorflow.python.tpu import functional as tpu_functional
...
flags.DEFINE_bool('enable_bf16', False, help='Use BF16 for activations.')
...
flags.DEFINE_string(
    'input_features', 'agz', help='Type of input features: "agz" or "mlperf07"')

flags.DEFINE_string(
    'input_layout', 'nchw', help='Layout of input features: "nhwc" or "nchw"')
  ...
  def initialize_graph(self):
    with self.sess.graph.as_default():
      ...
      if self.save_file is not None:
        tf.train.init_from_checkpoint(self.save_file, {'/': '/'})
...
def model_fn(features, labels, mode, params):
  ...
  Args:
      features: if input_layout == 'nhwc', a tensor with shape: [BATCH_SIZE,
        go.N, go.N, get_features_planes()]
          else, a tensor with shape: [BATCH_SIZE, get_features_planes(), go.N,
            go.N]
...
def freeze_graph(export_model_path,
                 use_trt=False,
                 trt_max_batch_size=8,
                 trt_precision='fp32'):
  output_names = ['policy_output', 'value_output']
  n = DualNetwork(FLAGS.work_dir)
...
def freeze_graph_tpu(export_model_path):
  ...
  if FLAGS.tpu_name:
    ...
    tpu_grpc_url = 'local'
  build_and_export_tpu(FLAGS.work_dir, export_model_path, tpu_grpc_url)
  metadata = make_model_metadata({
      'engine': 'tpu',
      'num_replicas': FLAGS.num_tpu_cores,
  })
  minigo_model.write_graph_def_tpu(
      metadata,
      os.path.join(
          export_model_path.rsplit('/', 1)[0],
          export_model_path.rsplit('/', 1)[1] + '.minigo'))


def make_model_metadata(metadata):
  for f in [
      'conv_width', 'fc_width', 'trunk_layers', 'use_SE', 'use_SE_bias',
...
def tpu_call(*args):
  ...
  def minigo_model_fn(features):
    ...
    if FLAGS.input_layout == 'nhwc':
      ...
      features = tf.reshape(features, [-1, get_features_planes(), go.N, go.N])
    else:
      raise ValueError('invalid input_layout "%s"' % FLAGS.input_layout)
    return model_fn(
        features,
  ...
  policy_output, value_output = tpu_functional.TPUPartitionedCall(
      args=tpu_subgraph.captured_inputs,
      device_ordinal=tpu_ops.tpu_ordinal_selector(),
      Tout=[o.type for o in tpu_subgraph.definition.signature.output_arg],
      f=tpu_subgraph)
  # Needed for BF16 conversion to latch on to.
  return (tf.identity(policy_output, name='tpu_policy_out'),
          tf.identity(value_output, name='tpu_value_out'))
...
def build_and_export_tpu(model_path, export_model_path, master):
  ...
  if FLAGS.enable_bf16:
    ...
    builder = tf.compat.v1.saved_model.Builder(export_model_path + '_pre_bf16')
  else:
