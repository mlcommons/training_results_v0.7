...
# pylint: disable=missing-docstring,deprecated-lambda,protected-access
...
from REDACTED.minigo import features
from REDACTED.minigo import go
from REDACTED.minigo import preprocessing
from REDACTED.minigo import symmetries
from REDACTED.minigo.tests import test_utils

TEST_SGF = ("(;CA[UTF-8]SZ[9]PB[Seth the best]PW[Andrew opens "
            "H4]KM[6.5]HA[0]RE[W+1.5]GM[1];B[fd];W[cf])")
...
class TestPreprocessing(test_utils.MinigoUnitTest):
  ...
  def create_random_data(self, num_examples):
    ...
    for _ in range(num_examples):
      feature = (256 * np.random.random([go.N, go.N, planes])).astype(np.uint8)
    ...
    with tf.Session() as sess:
      while True:
        try:
          ...
          recovered_data.append((pos_value, label_values["pi_tensor"],
  ...
  def assert_rotate_data(self, run_one, run_two, run_three):
    ...
    self.assertTrue(self.x_and_pi_same(run_two, run_three), "Not deterministic")
    self.assertFalse(
        self.x_and_pi_same(run_one, run_two), "Not randomly rotated")
    ...
    for (x, pi, v), (x2, pi2, v2) in zip(run_one, run_two):
      ...
      syms.extend(map(lambda r: self.find_symmetry(*r), zip(x, pi, x2, pi2)))
  ...
  def test_filter(self):
    ...
    # this will flake out very infrequently.  Use set_random_seed
  ...
  def test_make_dataset_from_sgf(self):
    ...
    expected_data = [(features.extract_features(start_pos, f),
