...
# pylint: disable=g-short-docstring-punctuation,g-doc-args,superfluous-parens,missing-docstring,g-complex-comprehension,unused-variable
...
class DummyNode(object):
  ...
  def __init__(self):
    self.parent = None
    self.child_N = collections.defaultdict(float)  # pylint: disable=invalid-name
    self.child_W = collections.defaultdict(float)  # pylint: disable=invalid-name


# pylint: disable=missing-docstring, g-doc-args,unused-argument, redefined-outer-name, invalid-name, g-doc-return-or-yield, g-short-docstring-space, g-short-docstring-punctuation
class MCTSNode(object):
  ...
  def __init__(self, position, fmove=None, parent=None):
    ...
    self.child_N = np.zeros([go.N * go.N + 1], dtype=np.float32)  # pylint: disable=invalid-name
    self.child_W = np.zeros([go.N * go.N + 1], dtype=np.float32)  # pylint: disable=invalid-name
  ...
  @property
  def child_Q(self):  # pylint: disable=invalid-name
    return self.child_W / (1 + self.child_N)

  @property
  def child_U(self):  # pylint: disable=invalid-name
    return ((2.0 * (math.log(
  ...
  @W.setter
  def W(self, value):  # pylint: disable=invalid-name
    self.parent.child_W[self.fmove] = value

  @property
  def Q_perspective(self):  # pylint: disable=invalid-name
    """Return value of position, from perspective of player to play."""
    return self.Q * self.position.to_play

  def select_leaf(self):  # pylint: disable=missing-docstring
    current = self
    pass_move = go.N * go.N
    while True:
  ...
  def add_virtual_loss(self, up_to):  # pylint: disable=g-doc-args,
    """Propagate a virtual loss up to the root node.

        Args:
            up_to: The node to propagate until. (Keep track of this! You'll need
              it to reverse the virtual loss later.)
    """
    ...
    self.W += loss  # pylint: disable=invalid-name
  ...
  def rank_children(self):
    ...
    ranked_children.sort(
        key=lambda i: (self.child_N[i], self.child_action_score[i]),
        reverse=True)
