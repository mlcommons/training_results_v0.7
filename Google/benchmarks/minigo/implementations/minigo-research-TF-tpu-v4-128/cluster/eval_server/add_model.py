...
# Lint as: python3
# pylint: disable=invalid-name,pointless-string-statement,line-too-long,missing-docstring,g-import-not-at-top,g-multiple-import,syntax-error,superfluous-parens,g-docstring-has-escape
...
def verify_params(model_path, model_flags, binary_path):
  ...
  assert model_flags.startswith(
      "flags: "), "model_flags must start with \"flags: \""
...
def add_model(argv):
  ...
  print("CBT: {" + FLAGS.cbt_project + "}:{" + FLAGS.cbt_instance + "}:{" +
        FLAGS.cbt_table + "}")
  assert FLAGS.cbt_project and FLAGS.cbt_instance and FLAGS.cbt_table, \
      "Must define --cbt_project, --cbt_instance, and --cbt_table"
  bt_table = (
      bigtable.Client(FLAGS.cbt_project, admin=True).instance(
          FLAGS.cbt_instance).table(FLAGS.cbt_table))
  assert bt_table.exists(), "Table({}) doesn't exist".format(FLAGS.cbt_table)
  # Check that model wasn't already uploaded.
  assert bt_table.read_row(
      player_name) is None, "{} already uploaded".format(player_name)
  ...
  print("Saving to {}".format(gcs_player_folder))
  ...
  for bin_file in [GTP_BIN, LIB_TF_FW_SO, LIB_TF_CC_SO]:
    copy_to_gcs(
        os.path.join(binary_base, bin_file),
  ...
  for status in response:
    ...
    if status.code is not 0:
      print("Failed to write {}".format(status))
