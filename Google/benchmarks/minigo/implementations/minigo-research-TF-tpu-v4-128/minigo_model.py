...
# pylint: disable=g-doc-args,missing-docstring,g-doc-exception,g-doc-return-or-yield,g-space-before-docstring-summary,unused-argument,g-short-docstring-punctuation, g-no-space-after-docstring-summary
...
def write_graph_def_tpu(metadata, dst_path):
  """Writes the metadata in Minigo format.
  ...
  for key, value in metadata.items():
    ...
    assert _is_supported_metadata_type(
        value), '%s: unsupported type %s' % (key, type(value).__name__)
  ...
  file_size = header_size + metadata_size
...
def read_model(path):
  ...
  with tf.io.gfile.GFile(path, 'rb') as f:
    ...
    if magic != MAGIC:
      raise RuntimeError('expected magic string %s, got %s' % (MAGIC, magic))

    version, file_size, metadata_size = struct.unpack('<QQQ',
                                                      f.read(HEADER_SIZE))
    if version != 1:
