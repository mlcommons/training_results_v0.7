...
def load_split(batch_size, train, dtype=tf.float32, image_size=IMAGE_SIZE,
  ...
  def shard_batch(batch):
    ...
    if batch_size % local_device_count != 0:
      ...
      clipped_batch = (batch_size // local_device_count) * local_device_count
      images = images[:clipped_batch]
      labels = labels[:clipped_batch]
    if space_to_depth:
      images = tf.reshape(images, [local_device_count, -1,
                                   image_size // 2, 2,
                                   image_size // 2, 2 * 3])
    ...
    if transpose_images:
      def numpy_transpose(x):
        return tf.convert_to_tensor(x._numpy().transpose([0, 2, 3, 4, 1]))  # pylint: disable=protected-access
  ...
  if train:
    ds = ds.cache()
    ds = ds.shuffle(16 * batch_size, seed=0, reshuffle_each_iteration=True)
    ds = ds.repeat()

  ds = ds.map(decode_example, num_parallel_calls=tf.data.experimental.AUTOTUNE)
  ds = ds.batch(batch_size, drop_remainder=train)

  ds = ds.map(shard_batch, num_parallel_calls=tf.data.experimental.AUTOTUNE)
