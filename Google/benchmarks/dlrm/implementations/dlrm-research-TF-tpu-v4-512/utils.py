...
def lr_fn(params, global_step):
  ...
  is_warmup = tf.cast(global_step < lr_warmup_steps, tf.float32)
  is_decay = tf.cast(global_step > decay_steps_float, tf.float32)
  is_middle = tf.cast(tf.equal(is_warmup + is_decay, 0.0), tf.float32)

  lr = is_warmup * warmup_lr + is_middle * adjusted_lr + is_decay * decay_lr
  return lr
