...
from REDACTED.transformer_lingvo.lingvo.core import base_model
from REDACTED.transformer_lingvo.lingvo.core import checkpointer
...
from REDACTED.transformer_lingvo.lingvo.core import metrics
from REDACTED.transformer_lingvo.lingvo.core import ml_perf_log as mlp_log
...
from REDACTED.tpu import compilation_result_pb2 as tpu_compilation_result
from REDACTED.tensorflow.python.tpu import tpu
from REDACTED.tensorflow.python.tpu import tpu_function
from REDACTED.tensorflow.python.tpu import training_loop as tpu_training_loop
...
def OnDeviceTrainAndEvalLoops(func):
...
class MLPerfTrainDecodeProgram(BaseProgram):
  ...
  def Params(cls):
    ...
    p.Define('num_epochs_per_session_run', 4, '')
    p.Define('ml_perf', None, 'MLPerf config')
    p.Define('warmup_seconds', 0, 'Sleep time before first infeed.')
    return p
  ...
  def __init__(self, params):
    ...
    self.num_epochs_per_session_run = params.num_epochs_per_session_run
    ...
    self._warmup_seconds = p.warmup_seconds
    self.spmd = True
  ...
  def _TrainAndDecodeLoop(self, sess):
    sess.run(self.train_and_decode_loop)
  ...
  def Run(self, sess):
    ...
    if self._ml_perf_log:
      ...
      self._ml_perf_epoch = int(global_step) // steps_per_epoch

    # Start TPU program thread.
    train_future = self._train_pool.apply_async(
        self._TrainAndDecodeLoop, args=(sess,))
    ...
    for e in range(self.num_epochs_per_session_run):
      epoch = self._ml_perf_epoch + e
      self.dec_metrics = self._decode_model_task.CreateDecoderMetrics()
      if not self._run_stop:
        if self._ml_perf_log:
          steps_per_epoch = self._ml_perf.steps_per_epoch
          self.SetStatusMessage('MLPerf epoch: %d' % epoch)

        for i in range(self._decode_steps_per_loop):
          metric_dict = sess.run(self.metric_dicts)
          if i == 0 and self._ml_perf_log:
            mlp_log.mlperf_print(
                'block_start',
                None,
                metadata={
          ...
          #for metric_values in metric_dicts:
          self._decode_model_task.PostProcessDecodeOut(metric_dict,
                                                       self.dec_metrics)
          ...
          if (mlperf_metric_value >
            ...
            if not self._run_stop:
              ...
              self.SetStatusMessage(
                  'MLPerf num_epochs: %d run_stop: %.2f, bleu: %.3f' %
                  (epoch + 1, self._run_stop, 100.0 * mlperf_metric_value))
    infeed_future.wait()
    train_future.wait()
...
def MLPerfProgramScheduleForTask(train_dataset_name,
                                 train_steps_per_loop,
                                 decode_dataset_name,
                                 decode_steps_per_loop,
                                 num_epochs_per_session_run,
                                 warmup_seconds=120):
  ...
  Args:
    ...
    decode_steps_per_loop: Number of steps to execute the decode program.
    num_epochs_per_session_run: Number of epochs per session.run
    warmup_seconds: Time to wait after compilation and before infeed.
  ...
  train_program_params.num_epochs_per_session_run = num_epochs_per_session_run

  train_program_params.dataset_name = train_dataset_name
  train_program_params.train_dataset_name = train_dataset_name
  train_program_params.decode_dataset_name = decode_dataset_name
  train_program_params.warmup_seconds = warmup_seconds
