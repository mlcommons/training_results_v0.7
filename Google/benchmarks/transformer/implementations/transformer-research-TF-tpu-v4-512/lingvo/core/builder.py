...
class Base(object):
  ...
  def _ActivationXlaShard(self, name, fn=tf.nn.relu, num_partitions=None):
    p = self.params

    def XlaShardRelu(i):
      if num_partitions:
        i = xla_sharding.split(i, 2, num_partitions, use_sharding_op=True)
      return fn(i)

    return self._Fn(name, XlaShardRelu)
