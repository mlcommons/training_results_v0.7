  ...
  def __init__(self, params):
    ...
    with tf.variable_scope(p.name + '/q'):
      ...
      for qdname in dir(p.qdomain):
        ...
        assert issubclass(
            qdparams.cls,
            QDomain), ('Expected quantized domain %s to extend QDomain' %
                       qdname)
...
class LinearClippingCapSchedule(BaseClippingCapSchedule):
  ...
  def Params(cls):
    ...
    p.Define('end_step', 15000, 'We reach end_cap by end_step.')
    p.Define(
        'start_cap', 8.0,
  ...
  def _Value(self, current_step):
    ...
    steps_ratio = (
        ...
        (end_step - start_step))
    rmax_tensor = (steps_ratio * p.end_cap + (1.0 - steps_ratio) * p.start_cap)
    return tf.cond(
