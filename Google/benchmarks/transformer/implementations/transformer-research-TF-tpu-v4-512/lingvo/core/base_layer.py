...
class BaseLayer(tf.Module):
  ...
  def CopyBaseParams(from_params, to_params):
    ...
    if from_params.xla_num_partitions is not None:
      to_params.xla_num_partitions = from_params.xla_num_partitions
  ...
  def AddFunction(self, name, f, replace=False):
    ...
    if not replace:
      if name in self._private_fns:
        raise AttributeError('Function "%s" is already defined on layer "%r"' %
                             (name, self))
