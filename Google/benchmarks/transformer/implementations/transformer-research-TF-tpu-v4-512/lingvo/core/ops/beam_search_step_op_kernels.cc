...
#include "REDACTEDsubmissions/training/v0_7/models/prod/transformer_lingvo/lingvo/core/ops/beam_search_step_op_kernels.h"

#include <cmath>

#include "REDACTEDsubmissions/training/v0_7/models/prod/transformer_lingvo/lingvo/core/ops/hyps.proto.h"
#include "REDACTEDsubmissions/training/v0_7/models/prod/transformer_lingvo/lingvo/core/ops/simple_vocab.h"
...
namespace debug {
static string IdsToStr(const proto2::RepeatedField<int32>& ids) {
...
void ComputeTopKPlusM(const std::vector<Hyp>& hyps, const Tensor& scores,
  ...
  Shard(kNumWorkers, workers, hyps_size, num_ids,
...
class BeamSearchStepOp : public OpKernel {
 public:
  explicit BeamSearchStepOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    ...
    OP_REQUIRES_OK(ctx,
                   ctx->GetAttr("local_eos_threshold", &local_eos_threshold_));
...
class HypsFromBeamSearchOuts : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    Shard(kNumWorkers, workers, num_hyps, seq_length * seq_length,
            ...
            for (int i = 0; i < seq_length; ++i) {
              for (int j = start; j < end; ++j) {
                ...
                if (t_done_hyps(i, j)) {
                  ...
                  // Walk through the token id matrix, and assemble id, score,
                  // and prev_hyp_id for each step.
