...
#include "REDACTEDsubmissions/training/v0_7/models/prod/transformer_lingvo/lingvo/core/ops/ascii_tokenizer.h"
#include "REDACTEDsubmissions/training/v0_7/models/prod/transformer_lingvo/lingvo/core/ops/simple_vocab.h"
#include "REDACTEDsubmissions/training/v0_7/models/prod/transformer_lingvo/lingvo/core/ops/tokenizer_op_headers.h"
...
class BpeIdsToWordsOp : public OpKernel {
 public:
  explicit BpeIdsToWordsOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    ...
    string contents;
    OP_REQUIRES_OK(
        ctx, ReadFileToString(Env::Default(), vocab_filepath_, &contents));
    std::vector<string> lines =
        str_util::Split(contents, '\n', str_util::SkipEmpty());
...
class BpeWordsToIdsOp : public OpKernel {
 public:
  explicit BpeWordsToIdsOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    ...
    OP_REQUIRES_OK(ctx, ctx->GetAttr("maxlen", &maxlen_));
    OP_REQUIRES_OK(
        ctx, ctx->GetAttr("tokenization_filepath", &tokenization_filepath_));
