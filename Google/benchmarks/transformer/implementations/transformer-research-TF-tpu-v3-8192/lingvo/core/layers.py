...
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import bn_layers
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import computation_cost
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import constants
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import conv_layers_with_time_padding
    ...
    for i in range(num_layers):
      ...
      if i == 0:
        if p.xla_num_partitions:
          layer_out = xla_sharding.split(
              layer_out, 2, p.xla_num_partitions, use_sharding_op=True)
...
class SharedSoftmaxLayer(SimpleFullSoftmax):
  ...
  def EmbLookup(self, theta, ids):
    ...
    wm = tf.cast(wm, tf.bfloat16)
    if p.xla_num_partitions:
      wm = xla_sharding.split(wm, 0, p.xla_num_partitions, use_sharding_op=True)
