...
from REDACTED.tensorflow.python.ops import io_ops
from REDACTED.tensorflow.python.tpu import tpu_embedding as tpu_embedding_lib
...
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import datasource
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import hyperparams
from REDACTED.tensorflow_models.mlperf.models.rough.transformer_lingvo.lingvo.core import input_generator_helper as ig_helper
...
class BaseInputGenerator(base_layer.BaseLayer):
  ...
  def GetPreprocessedInputBatch(self, task_id=None):
    ...
    """
    return self._PreprocessInputBatch(self._InputBatch(task_id=task_id))
  ...
  def CreateTpuEnqueueOps(self):
    ...
    for task_id in range(num_infeed_hosts):
      ...
      self._per_host_device.append(host_device)
    ...
    def TPUOrdinalFunction(shard_index_in_host):
      ...
      if device_assignment:
        ...
        replica = device_assignment.lookup_replicas(task_id,
                                                    0)[shard_index_in_host]  # pylint: disable=cell-var-from-loop
  ...
  @property
  def per_host_device(self):
    return self._per_host_device
...
class BaseInputGeneratorFromFiles(BaseInputGenerator):
  ...
  def _BuildDataSourceWithMetadata(self, task_id=None):
    ...
    if not p.file_datasource and p.file_pattern:
      ...
      ret.data = self._DataSourceFromFilePattern(
          p.file_pattern, task_id=task_id)
    else:
      tf.logging.info(
      ...
      ret = self.datasource.BuildDataSource(
          self._DataSourceFromFilePattern, task_id=task_id)  #, task_id=task_id)
  ...
  def _BuildDataSource(self, task_id=None):
    """Read and return input batch from `p.file_pattern`.
    ...
    """
    return self._BuildDataSourceWithMetadata(task_id=task_id)['data']
