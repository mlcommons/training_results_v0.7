...
DEFINE_string(pause_file, "",
              "If non-empty, specifies a path to a file whose presence is "
              "checked for periodically when run_forever=true. If the file "
              "exists the selfplay process will pause until this file "
              "disappears.");
...
bool SelfplayGame::MaybeQueueInference(MctsNode* leaf, InferenceCache* cache,
  ...
  if (cache->TryGet(cache_key, leaf->canonical_symmetry, inference_sym,
    ...
    hit ++;
    if ((hit%10000)==0) {
      MG_LOG(INFO) << "Inference cache hit " << hit << " times. Missed " << miss << " times. "
                   << " Hitrate = " << (hit*100)/(hit+miss) << "%";
    }
...
void SelfplayThread::Run() {
  int last_num_games_finished = 0;
  WTF_THREAD_ENABLE("SelfplayThread");

  searches_.resize(FLAGS_parallel_search);
  auto begin = absl::Now();
  while (!selfplay_games_.empty()) {
    while (file::FileExists(FLAGS_pause_file)) {
      MG_LOG(INFO) << "Pausing because " << FLAGS_pause_file << " was found";
      sleep(1);
    }
    StartNewGames();
    SelectLeaves();
    ...
    PlayMoves();
    if (num_games_finished_ > last_num_games_finished) {
      auto duration = absl::ToDoubleSeconds(absl::Now() - begin);
      MG_LOG(INFO) << "Throughput: " << num_games_finished_ / duration
                   << " games/s";
      last_num_games_finished = num_games_finished_;
    }
  }
