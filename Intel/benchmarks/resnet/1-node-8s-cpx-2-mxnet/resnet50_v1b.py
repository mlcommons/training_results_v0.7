...
def resnet(units, num_stages, filter_list, num_classes, image_shape, bottle_neck=True, bn_mom=0.9, workspace=256, dtype='float32', memonger=False, label_smoothing=0.1):
    ...
    return mx.sym.SoftmaxOutput(data=fc1, name='softmax', smooth_alpha=0.1)

def get_symbol(num_classes, num_layers, image_shape, conv_workspace=256, dtype='float32', label_smoothing=0.1, **kwargs):
    """
    Adapted from https://github.com/tornadomeet/ResNet/blob/master/symbol_resnet.py
    (Original author Wei Wu) by Antti-Pekka Hynninen
    Implementing the original resnet ILSVRC 2015 winning network from:
