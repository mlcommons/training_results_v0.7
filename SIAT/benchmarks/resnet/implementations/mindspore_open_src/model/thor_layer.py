...
class Conv2d_Thor(_Conv):
    ...
    def __init__(self,
        ...
        if self.matrix_A_dim == 147 and self.batch_size==256:
            self.layer_thor = False
        else:
            self.layer_thor = True
    ...
    def save_gradient(self, dout):
        ...
        dout = self.reshape(dout, (dout_shape[0] * dout_shape[1] * dout_shape[2], dout_shape[3]))
        normalizer = dout_shape[0] * dout_shape[1] * dout_shape[2]
        fm_size = dout_shape[1] * dout_shape[2]
        ...
        damping = self.mul(damping_step, 1.0 / fm_size)
    ...
    def construct(self, x):
        if self.thor:
            if self.layer_thor:
                ...
                damping_step = self.cast(damping_step, mstype.float32)
                damping = self.mul(damping_step, self.batch_size / normalizer)
                damping = self.sqrt(damping)
...
class Dense_ThorNoBN(Cell):
    ...
    def __init__(self,
        ...
        self.multiplier = Parameter(Tensor(np.ones([1]), dtype=mstype.float16), requires_grad=True, name='multiplier')
