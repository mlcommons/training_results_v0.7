...
NVIDIA DGX-2H single node submission are in the `config_DGX2.sh` script.
...
instructions in [NVIDIA DGX-2H (single
node)](#nvidia-dgx-2h-single-node) explain how that is done.
