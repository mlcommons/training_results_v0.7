...
def ensure_file_exists(src, verbose=False):
    """Touch file if it doesn't exist."""

    if not tf.io.gfile.exists(src):
       if(verbose): print('Creating file {}'.format(src))
       with tf.io.gfile.GFile(src, 'wb') as f:
           f.write('Abort\n')


def ensure_dir_exists(src, verbose=False):
    """Make directory src if it doesn't already exists."""

    if not tf.io.gfile.exists(src):
       if(verbose): print('Creating {}'.format(src))
       tf.io.gfile.makedirs(src)

def remove_tree(src, verbose=False):
    """Removes everything under src."""

    if tf.io.gfile.exists(src):
       if verbose: print('Deleting {}'.format(src))
       tf.io.gfile.rmtree(src)

def copy_file(src, dst, verbose=False):
    """Copies src file to dst."""

    if verbose: print('Copying {} to {}'.format(src, dst))
    tf.io.gfile.copy(src, dst)

def copy_dir_tree(src, dst, verbose=False):
    """Copies directory structure under src to dst."""

    if verbose:
        print('Copying dir-structure {} to {}'.format(src, dst))
...
async def checked_run(cmd, env=None, process_will_abort=False):
    ...
    with logged_timer('{} finished'.format(get_cmd_name(cmd))):
        ...
        await p.wait()
        output = '\n'.join(lines)[:-1]
        if p.returncode and not process_will_abort:
