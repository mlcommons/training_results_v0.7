...
# 2. Directions
### Steps to launch training for NVIDIA DGX2H ( 32 nodes )
...
### Run benchmark with SLURM for NVIDIA DGX2H ( 32 nodes )
```
    # Data from DATADIR is mounted to /data/ in docker.
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGX2_multi_32nodes.sh. 
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=DGX2_multi_32nodes sbatch -N 32 -t $WALLTIME run.sub
```
