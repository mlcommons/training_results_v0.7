...
# 2. Directions
### Steps to launch training for NVIDIA DGX1 
...
### Run benchmark with docker for NVIDIA DGX1 
```
    # nvidia-docker: Note that performance or functionality may vary from the tested SLURM instructions
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGX1.sh. 
    ...
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=DGX1 bash run_with_docker.sh

```

### Run benchmark with SLURM for NVIDIA DGX1 
```
    # Data from DATADIR is mounted to /data/ in docker.
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGX1.sh. 
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=DGX1 sbatch -N 1 -t $WALLTIME run.sub
```
