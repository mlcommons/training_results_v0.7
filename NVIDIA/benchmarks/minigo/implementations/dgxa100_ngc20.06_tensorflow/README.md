...
```
    ...
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=DGXA100 bash run_with_docker.sh
...
```
    ...
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGXA100.sh. 
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=DGXA100 sbatch -N 1 -t $WALLTIME run.sub
```
