...
# 2. Directions
### Steps to launch training for NVIDIA DGXA100 ( 2 nodes )
...
### Run benchmark with SLURM for NVIDIA DGXA100 ( 2 nodes )
```
    # Data from DATADIR is mounted to /data/ in docker.
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGXA100_multi_2nodes.sh. 
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=DGXA100_multi_2nodes sbatch -N 2 -t $WALLTIME run.sub
```
